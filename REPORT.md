...
**Architecture.** The frontend is a single-page React application built with Vite. Routing is handled by React Router with a shared Layout component (navigation + footer) wrapping all pages. State management uses React hooks. Tailwind CSS with custom CSS variables handles theming.

## Additional Data Sources

The following publicly available data sources were used to populate the platform:
...
- Prince George's County Dept of Social Services (princegeorgescountymd.gov)
...
- Virginia Federation of Food Banks (vafoodbanks.org)
- Arlington County Food Assistance (arlingtonva.us)
- UMD Extension Food Access Resources (extension.umd.edu)
...
- Prince George's County Open Data (data.princegeorgescountymd.gov)
...
1. **Scaffolding.** Initial prompts established the project structure: API client configuration, entity definitions, component stubs, page stubs, and UI component library.

2. **Component migration.** UI component code from a previous prototype was pasted into the chat and Kiro formatted and placed each file correctly. This included 30+ UI components.

3. **Data layer replacement.** A key challenge was migrating from external API calls to a fully local data layer. This required identifying all import references, creating a data module, and updating every page component.
...
5. **Branding and originality.** Prompts directed renaming and ensuring no plagiarized content remained.
...
**Current limitations:**
- Data is static — updates require code changes rather than pulling from a live database.
- ZIP code search is a simple string match, not geographic proximity.
- The inquiry form stores submissions in memory only.
- No user authentication or saved preferences.
- No map visualization despite having address data.

**Future improvements:**
- Connect Python scraping scripts to a scheduled job that updates a database. The frontend would fetch from this database instead of a static file.
- Use the browser Geolocation API with a geocoding service to sort results by distance.
- Add a Leaflet or Mapbox map view showing pantry locations as pins.
- Store form submissions in a database and send email notifications.
- Support multiple languages (at minimum English and Spanish).
- Conduct accessibility testing for WCAG 2.1 AA compliance.
- Train a model on food insecurity indicators to predict underserved communities and prioritize resource allocation.

## Conclusion

Sustainable Food Hub demonstrates that publicly available food assistance data can be aggregated into a usable, searchable interface that serves families, donors, and volunteers across the DMV region. The platform draws from 20+ verified public data sources and presents information in a clean, accessible format. While the current implementation uses static data, the architecture supports future expansion to live data feeds, geographic search, and predictive analytics.
